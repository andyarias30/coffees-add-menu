...
export default function AddCoffee({ setCoffees }){
    ...
    useEffect(() => {
        getCoffees()
    }, []) // one time , after components mounts, get the coffees

    const handleSubmit = (e) => {
        e.preventDefault()
        //gather from data
       // e.target is the form
       //e.target.name is the imput "name"
       //e.target.name.value is ther value of the input "name"
       ...
       const recipe =  e.target.recipe.value
        ...
        .then(data => {
            ...
            if(data.message ==="Success!"){
                //our coffee was added succesfully
